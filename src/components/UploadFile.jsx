...
const UploadFile = () => {
      ...
      if (file) {
        if (!file.type.startsWith('image/')) {
          ...
          setUploadImage({ ...uploadImage, 
            fileUpload: URL.createObjectURL(file),
            isUploaded: true,
            });
        }
      }
  ...
  return (
    ...
    <GoInfo className={`${uploadImage.isError && 'text-secondary'}`} />

      {uploadImage.isError ? uploadImage.error && <span className='text-xs pl-2 text-red-400'>{uploadImage.error}</span> :
      <span className='text-xs pl-2'>Upload your photo (JPG or PNG, max size: 500KB).</span>
    }
    </div>
